...
def control_hardware_simulation(activate: bool, current_density: float):
    if activate and not st.session_state.last_hardware_state:
        ...
        print(">> 【マイコン模擬信号】 'H' (LED点灯) を送信しました。")
        print("=" * 50 + "\n")
        ...
        print(">> 【マイコン模擬信号】 'L' (デバイス停止) を送信しました。")
        print("=" * 50 + "\n")
        st.session_state.last_hardware_state = False


# ==============================================================================
# 6. Streamlit UI のレイアウト構築
# ==============================================================================
st.title("🛡️ 壁内人口密度監視システム (超精密AI・シミュレーション版)")

st.sidebar.header("⚙️ システム設定")
run_system = st.sidebar.checkbox("システムを起動（ループ開始）", value=False)
MAX_CAPACITY = st.sidebar.slider("エリア内の最大許容人数 (100%基準)", 5, 300, 150) # ← 上限を300人に拡大
...
if run_system:
    ...
    if frame is None:
        st.error(f"【エラー】画像ファイル '{IMAGE_PATH}' が読み込めませんでした。")
        st.stop()
    ...
    if st.session_state.final_people_boxes is None:
        with st.spinner("AIが精密スキャンを実行中..."):
            ...
            for r in range(BASE_ROWS):
                for c in range(BASE_COLS):
                    if rough_grid[r, c]:
                        # 【チューニング1】境界バッファを35pxに広げて隣接マスとの見落としを防止
                        buffer = 35
                        ...
                        # 【チューニング2】確信度(conf)をしきい値ギリギリの 0.04 まで引き下げて豆粒人間を強引に拾う
            ...
            if len(all_boxes) > 0:
                ...
                nms_indices = cv2.dnn.NMSBoxes(
                    ...
                    nms_threshold=0.5  # 重複判定を少し緩めて密集した人を残す
                )
    ...
    with metrics_placeholder.container():
        st.metric(label="現在の壁内人数 (AI精密カウント)", value=f"{people_in_wall} 人")
